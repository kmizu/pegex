...
class PegexBasicSpec extends AnyFeatureSpec {
  feature("PEGEXs") {
    ...
    scenario("identifiers") {
      val ident = """\g'IdentStart'\g'IdentRest'*$;
      IdentStart=[a-zA-Z_];
      IdentRest=\g'IdentStart'|[0-9];
                  """.e
    ...
    scenario("a context sensitive language") {
      val csl = """(?=#{A}(?!b))a+\g'B'$; A=a#{A}?b; B=b#{B}?c;""".e
  ...
  feature("PEGEX with back reference"){
    ...
    scenario("XMLs") {
      val minXml = """#{E}$; E=<(?<tag>#{I})>#{E}*</\k<tag>>; I=[a-z]+;""".e
